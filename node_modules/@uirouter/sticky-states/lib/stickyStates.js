...
(function (factory) {
    ...
    var isChildOfAny = function (_parents) {
        return function (node) { return _parents.map(function (parent) { return isChildOf(parent)(node); }).reduce(core_1.anyTrueR, false); };
    };
    ...
    var isDescendantOfAny = function (ancestors) { return function (node) {
        return ancestors.map(function (ancestor) { return isDescendantOf(ancestor)(node); }).reduce(core_1.anyTrueR, false);
    }; };
    /**
    ...
    var notFoundInPath = function (path) { return core_1.not(findInPath(path)); };
    /** uirouter/core 5.x/6.x compatibility code */
    var cloneNode = function (node) {
        var n = node;
        return (core_1.isFunction(n.clone) && n.clone()) || core_1.PathNode.clone(node);
    };
    var applyParamsFromPath = function (path, dest) {
        ...
        if (!sourceNode)
            throw new Error("Could not find matching node for " + dest.state.name + " in source path [" + path
                ...
                .join(', ') + "]");
        return core_1.extend(cloneNode(dest), { paramValues: sourceNode.paramValues });
    };
    /**
    ...
    var StickyStatesPlugin = /** @class */ (function (_super) {
        ...
        StickyStatesPlugin.prototype._calculateStickyTreeChanges = function (trans) {
            ...
            var inactiveFromPath = tc.retained.concat(tc.entering.map(findInPath(inactives))).filter(core_1.identity);
            ...
            trans.onSuccess({}, function () {
                tc.exiting.map(findInPath(inactives)).forEach(core_1.removeFrom(inactives));
                tc.entering.map(findInPath(inactives)).forEach(core_1.removeFrom(inactives));
                tc.reactivating.map(findInPath(inactives)).forEach(core_1.removeFrom(inactives));
            ...
            exitSticky.filter(function (node) { return !findInPath(tc.exiting)(node); }).forEach(core_1.pushTo(tc.exiting));
            // Also process the active sticky states that are about to be inactivated, but should be exited
            exitSticky.filter(findInPath(tc.inactivating)).forEach(core_1.removeFrom(tc.inactivating));
            return tc;
        };
