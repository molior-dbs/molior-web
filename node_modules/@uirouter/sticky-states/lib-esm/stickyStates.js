...
import { PathUtils, PathNode, UIRouterPluginBase, TransitionHookPhase, TransitionHookScope, tail, isString, isArray, inArray, removeFrom, pushTo, identity, anyTrueR, assertMap, uniqR, defaultTransOpts, isFunction, not, extend, } from '@uirouter/core';
...
var isChildOfAny = function (_parents) {
    return function (node) { return _parents.map(function (parent) { return isChildOf(parent)(node); }).reduce(anyTrueR, false); };
};
...
var isDescendantOfAny = function (ancestors) { return function (node) {
    return ancestors.map(function (ancestor) { return isDescendantOf(ancestor)(node); }).reduce(anyTrueR, false);
}; };
/**
...
var notFoundInPath = function (path) { return not(findInPath(path)); };
/** uirouter/core 5.x/6.x compatibility code */
var cloneNode = function (node) {
    var n = node;
    return (isFunction(n.clone) && n.clone()) || PathNode.clone(node);
};
...
var StickyStatesPlugin = /** @class */ (function (_super) {
    ...
    StickyStatesPlugin.prototype._calculateStickyTreeChanges = function (trans) {
        ...
        exitSticky.filter(function (node) { return !findInPath(tc.exiting)(node); }).forEach(pushTo(tc.exiting));
